...
$("#travelInformation").on("submit", function(event) {
    ...
    let map = $("<img>");
    map.attr("src", "https://maps.googleapis.com/maps/api/staticmap?center=" + valLocation + "&size=1000x300&key=AIzaSyDg0UKan_KlUTWl4PlNyMfnkTIpONTAfN0");
    $("#mapPlace").append(map);
                ...
                if (avgTemp < 16) {
                    if (valType === "beach") {
                        ...
                        for (let i = 0; i < resultList.length; i++) {
                            ...
                            let showQtdItem = $(
                                "<input class='qtd-item' data-item='" +
                                item +
                                "' value='1' type='number'>"
                            );
